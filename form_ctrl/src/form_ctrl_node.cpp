...
{
private:
  vector<string> rob_names_, rob_ctrl_topic_names_,
                           rob_ang_vel_topic_names_;

  // node handle and Subscribe and Publish topics
  ros::NodeHandle nh_;
  ros::Subscriber pos_world_sub_, pos_image_sub_;
  vector<ros::Publisher> ctrl_cmd_pub_array_, ang_vel_pub_array_;

  // variables used for formation control and PID controller
  vector<double> ctrl_, vd_, thte_, thtc_, thtc_old_, thtr_, tht0_old_,
                      tht0_new_, a_hat_old_, a_hat_new_, b_hat_old_, b_hat_new_,
                      error_sum_, vm_new_, vm_old_, vel_, ang_;
  vector<MatrixXd> pos_world_history_, pos_image_history_;
  double t_old_, t_new_, v_est_;
  int mem_, mem_filled_, num_rob_, thresh_pix_;

  int num_rows, idx;
  // variables for robot position
  std_msgs::Float64MultiArray pos_world_new_, pos_image_new_;
...
public:
FormControl()
:  window_name("heading command"), window_name2("reconstruction"), first_time(true),
  num_rows(-1), use_adaptive_vel_ctrl(false), use_PID_ctrl(true),
  spinner(2)
{
  double L [6][6];
}

~FormControl() {   // class destructor
    // file stream. To be removed for final version
    outfile.close();
}
  ...
  std_msgs::Float64MultiArray get_pos_world_new()   {
    return pos_world_new_;
  }

  std_msgs::Float64MultiArray get_pos_image_new() {
...
private:
  ...
  void spawn(){
    ...
    thread form_ctrl_thread = thread(&FormControl::form_ctrl, this);
    ...
    pos_image_sub_ = nh_.subscribe("/locs/ordered", 1, &FormControl::pos_image_callback, this);
    pos_world_sub_ = nh_.subscribe("/pos/world", 1, &FormControl::pos_world_callback, this);
  ...
  void populateRobots(){
    ...
    ros::param::get("/form_ctrl_node/Rob_Params/num_rob", num_rob_); // number of robots
    for (auto i = 0; i < num_rob_; ++i)  {
      std::string rob_id = "sph" + std::to_string(i);
      std::string rob_name;
      ros::param::get("/form_ctrl_node/Rob_Params/"+rob_id, rob_name);
      rob_names_.push_back(rob_name);
      rob_ctrl_topic_names_.push_back("/" + rob_names_[i] + "/cmd_vel");
      rob_ang_vel_topic_names_.push_back("/" + rob_names_[i] + "/set_angular_velocity");
      ctrl_cmd_pub_array_.push_back(nh_.advertise<geometry_msgs::Twist>(rob_ctrl_topic_names_[i], 1));
      ang_vel_pub_array_.push_back(nh_.advertise<std_msgs::Float32>(rob_ang_vel_topic_names_[i], 1));
    }
    pos_image_sub_ = nh_.subscribe("/locs/ordered", 1, &FormControl::pos_image_callback, this);
    pos_world_sub_ = nh_.subscribe("/pos/world", 1, &FormControl::pos_world_callback, this);
  }
  ...
  template <typename T>
  void retrieveParam(std::string base, std::string param, T value) const  {
    ros::param::get(base + param, value);
  }

  void form_ctrl() {
      int iitr = 0;
      for (int num_rob = 0; num_rob < num_rob_; ++num_rob)  {
        iitr = --itr * num_rob_ + (++num_rob);
        if (iitr == 1) {// initialize some variables in first iteration
          tht0_new_[num_rob] = 0;   tht0_old_[num_rob] = 0;
          thtc_[num_rob] = 0;       thtc_old_[num_rob] = 0;
        }
  ...
  void initParams(){
    ...
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "vd_gain", vd_gain_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "v_gain_", v_gain_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "kh_", kh_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "thresh_pix_", thresh_pix_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "vmax_", vmax_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "km_", km_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "ka_", ka_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "a_bound_", a_bound_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "b_bound_", b_bound_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "gain_", gain_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "speed_gain_", speed_gain_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "kp_", kp_);
    retrieveParam("/form_ctrl_node/Gains_Thresholds/", "ki_", ki_);
    retrieveParam("/form_ctrl_node/Delay/", "delay_", delay_);

    namedWindow(window_name, WINDOW_NORMAL);
    resizeWindow(window_name, 640, 480);
    namedWindow(window_name2, WINDOW_NORMAL);
    resizeWindow(window_name2, 400, 400);

    std::string package_name = ros::package::getPath("form_ctrl");
    outfile.open(package_name + "/" + "config" + "/" + "pub_data.txt");
  }
  ...
  void form_ctrl_free_scale()   // free scale formation control function
  {
      if (first_time)
      {
        ...
        double sep_ang = 360 / num_rob_;
        double q_ang [num_rob_];
        for (int i = 0; i < num_rob_; ++i)        {
          q_ang[i] = sep_ang*i;
        }
        ...
        int num_graphs = 1;
        double adj [this->num_rob_][this->num_rob_][num_graphs] = {0};
        for (int i = 0; i < this->num_rob_; ++i)
        {
        ...
        nh_.getParam("/form_ctrl_node/CVX_Optimization/rows", num_rows);
      ...
      MatrixXd q;
      q.resize(2*num_rob_,1);
      double q_max = 0;
      for (int i = 0; i < 2*num_rob_; ++i)
      {
        q(i) = pos_world_new_.data[i];
...
int main(int argc, char **argv)
{
  ...
  // while (fc.get_pos_world_new().data.empty() || fc.get_pos_image_new().data.empty())
  // {
  //         // ros::spinOnce();
  // }
  // at this point we have data in pos_world_new_ and pos_image_new_

  fc.run(); // apply formation control
